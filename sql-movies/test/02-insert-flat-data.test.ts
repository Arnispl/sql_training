...
const insertMovies = (movies: Movie[]) => {
  const values = movies
    .map(movie => {
      return `('${escape(movie.imdbId)}', '${escape(movie.popularity.toString())}', '${escape(movie.budget.toString())}', '${escape(movie.budgetAdjusted.toString())}', '${escape(movie.revenue.toString())}', '${escape(movie.revenueAdjusted.toString())}', '${escape(movie.originalTitle)}', '${escape(movie.homepage)}', '${escape(movie.tagline || '')}', '${escape(movie.overview)}', '${escape(movie.runtime.toString())}', '${escape(movie.releaseDate)}')`;
    })
    .join(", ");
